...
\end{itemize}
\vspace{0.5em}
\small Media de \textbf{3 semillas} (180 modelos). Significativo \textbf{entre} grupos, \textbf{no dentro}: GAT vs GCN $p=0{,}17$; GraphSAGE vs TAGCN $p=0{,}10$.
\end{column}
...
\end{itemize}
\vspace{0.3em}
{\scriptsize\color{muted} Valores exactos e IC en la Tabla de respaldo (anexo).}
\end{column}
\begin{column}{0.48\textwidth}\centering
\figcard{disociacion_eb.png}
\figcap{Plausibilidad de aristas vs Fidelity+ por explicador (eje sintético; IC 95\%; azar de plausibilidad $=0{,}40$). \emph{Fuente: elaboración propia.}}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}\small
  ...
  \item De 1:1 a 1:100, la estabilidad media va de $0{,}709$ (nativo) a $0{,}741$: amplitud de \textbf{tres centésimas}.
  \item Sin punto de quiebre, sin pico en 1:50, sin paradoja del nativo.
...
\figcap{Estabilidad media por escenario de desbalance (1:1 a 1:100). \emph{Fuente: elaboración propia.}}
\end{column}
\end{columns}
\end{frame}
...
\midrule
\textbf{H1.} La estabilidad se degradaría con el desbalance. & \textcolor{coral}{\textbf{Refutada}} & Perfil plano: amplitud 0,03 (0,709 nativo a 0,741). \\
...
\begin{enumerate}\small
  ...
  \item \textbf{\color{amber}El explicador es la palanca dominante}; el desbalance y el balanceo son secundarios ($\eta^2 < 0{,}02$).
...
\midrule
\textbf{O1.} Desbalance vs robustez explicativa & No es el factor dominante. Perfil plano, balanceo despreciable. La palanca real es el \textbf{explicador}. \\
...
\textbf{O3.} Estrategias de balanceo & Despreciable en las tres dimensiones ($\eta^2 < 0{,}02$). Elegible por rendimiento. \\
...
\begin{center}
\small
\tabcap{Plausibilidad de aristas y Fidelity+ por explicador (eje sintético, g0, 3 semillas).}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{lcc}
\toprule
\textbf{Explicador} & \textbf{Plausibilidad (aristas)} & \textbf{Fidelity+} \\
\midrule
GNNExplainer & 0,50 & \textbf{0,56} \\
PGExplainer & \textbf{0,80} & 0,11 \\
GNNShap & 0,15 & 0,46 \\
\midrule
Azar (baseline) & 0,40 & --- \\
\bottomrule
...
\end{center}
\vspace{0.5em}
{\footnotesize PGExplainer supera a GNNExplainer en plausibilidad: Wilcoxon pareado $p\approx 2{,}6\times10^{-35}$ ($n=225$ parejas). \textbf{GNNShap} es el más estable internamente entre ejecuciones, aunque no lidere plausibilidad ni fidelidad.}
\end{frame}
...
\end{center}
\vspace{0.5em}
{\footnotesize \textbf{\color{teal}GNNExplainer} es el único que discrimina entre arquitecturas: PGExplainer degenera (Spearman nulo) y GNNShap se satura cerca de 0,95, indistinguible. Además es el explicador común a los dos ejes, lo que hace comparable la partición.}
\end{frame}
