...
\begin{itemize}\scriptsize
  \item Tamaño de efecto $\eta^2 = 0{,}13$ (grande), el mismo que reporta el eje sintético.
  ...
  \item Los $p$ dentro de grupo ($0{,}19$; $0{,}08$) son no pareados; el Wilcoxon \textbf{pareado} de la lámina previa ($0{,}17$; $0{,}10$) coincide: ninguna vía rechaza.
\end{itemize}
\end{column}
...
\end{center}
\vspace{0.3em}
{\scriptsize n/d = métricas que el explicador no produce por diseño (PGExplainer no ordena \emph{features}; GNNShap no genera máscara de aristas). Contra su propia línea base, PGExplainer duplica el azar en aristas y GNNExplainer lo sextuplica en \emph{features}. PGExplainer supera a GNNExplainer en plausibilidad de aristas (Wilcoxon pareado $p\approx 2{,}6\times10^{-35}$, $n=225$). \textbf{GNNShap} es el más estable internamente ($0{,}98$).}
\end{frame}
...
\end{center}
\figcap{Curvas ROC (izquierda) y precisión--recall (derecha), validación vs test, mismos modelos. El ROC cae de 0,88 a 0,65, pero la precisión--recall se desploma casi a la tasa base de ilícitas: por eso la métrica primaria es PR-AUC, no ROC-AUC. \emph{Fuente: elaboración propia.}}
\end{frame}
