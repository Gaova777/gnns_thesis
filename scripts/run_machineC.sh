#!/usr/bin/env bash
# Machine C launcher — RTX 3050 Laptop (4GB)
# Configs: GCN + GraphSAGE, scenarios 1:50 and 1:100 = 36 configs
set -euo pipefail

MACHINE="C"
CONFIG="configs/experiment_machineC.yaml"
RESULTS_DIR="./results_machineC"
LOG_DIR="logs"
LOG_FILE="${LOG_DIR}/machineC.log"
HEARTBEAT_LOG="${LOG_DIR}/machineC_heartbeat.log"
EXPECTED_CONFIGS=36
LOCK="/tmp/gnns_thesis_machineC.lock"
MAX_RETRIES=3
...
if torch.cuda.is_available():
    ...
    if vram_gb < 5.0:
        print(f"NOTE: <5GB VRAM — GNNShap will auto-reduce to 25 samples on OOM")
else:
    print("WARNING: No CUDA GPU detected — running on CPU (will be slow)")
    print("GPU detected:   CPU / VRAM total: N/A / SHAP mode: normal")
PYEOF
echo "Configs to run: ${EXPECTED_CONFIGS} (GCN+SAGE × scenarios 1:50,1:100 × 3 balancing × 3 explainers)"
echo ""
