@echo off
REM Machine C launcher (Windows) — RTX 3050 Laptop (4GB)
REM GCN + GraphSAGE, escenarios 1:50 y 1:100 = 36 configs
REM
...
set MACHINE=C
set CONFIG=configs/experiment_machineC.yaml
set LOG_DIR=logs
set LOG_FILE=%LOG_DIR%\machineC_windows.log
...
echo.
echo Configs a correr: 36 (GCN+SAGE x escenarios 1:50,1:100 x 3 balanceos x 3 explainers^)
echo [NOTE] VRAM 4GB: GNNShap reducira muestras a 25 automaticamente si hay OOM
echo.
...
if %ERRORLEVEL% EQU 0 (
    ...
    wsl bash scripts/run_machineC.sh 2>&1 | tee "%LOG_FILE%"
    goto :done
)
