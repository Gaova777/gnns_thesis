...
Este capítulo traslada los fundamentos del Capítulo 3 a un entorno experimental concreto sobre el \textit{Elliptic Dataset}, el conjunto de transacciones reales de Bitcoin que sirve de referencia en la detección de actividad ilícita. El capítulo describe primero el preprocesamiento y una característica estructural del grafo que condiciona todo el estudio, que es su dispersión extrema, y luego presenta el espacio factorial de configuraciones que se evaluó. A continuación se reportan el rendimiento predictivo y un fenómeno central del dominio, el colapso del desempeño entre la partición de validación y la de test, que obliga a estudiar la estabilidad de las explicaciones sobre validación. Finalmente, el capítulo documenta una corrección metodológica en la forma de generar las explicaciones que reordena el ranking de estabilidad entre arquitecturas y que constituye una de las contribuciones centrales de la tesis.
...
El comportamiento de la estabilidad a lo largo de los escenarios de desbalance confirma que este factor no gobierna la reproducibilidad de las explicaciones. El análisis se realiza sobre la matriz replicada con las tres semillas de modelo, el mismo conjunto que sostiene la partición por arquitectura de la sección anterior, de manera que la lectura por escenarios no queda apoyada en una inicialización única. Los valores medios recorren un rango estrecho, de 0,696 en el escenario 1:1 a 0,765 en el escenario 1:50, pasando por 0,736 en 1:10, por 0,745 en 1:100 y por 0,760 en el escenario nativo, como ilustra la Figura \ref{fig:escenario} y detalla por arquitectura la Tabla \ref{tab:elliptic-stab-scen}. La amplitud total entre escenarios, de siete centésimas, es inferior a la que separa a las arquitecturas, que asciende a once centésimas entre GAT y TAGCN, de modo que el escenario resulta el menos influyente de los dos factores.

La comparación formal entre escenarios respalda esa lectura. La prueba de Kruskal-Wallis sobre el factor escenario arroja un estadístico de 6,29 con un valor p de 0,178, de manera que no se rechaza la igualdad entre los cinco niveles, y el tamaño de efecto asociado, de 0,051, corresponde a un efecto pequeño frente al 0,126 que alcanza la arquitectura sobre la misma métrica. El sentido de la comparación importa, porque la hipótesis de partida anticipaba un deterioro de la estabilidad a medida que la clase ilícita se volviera más escasa, y lo que se observa no es un deterioro sino una variación sin dirección, con el valor más bajo en el escenario 1:1, que es el más equilibrado de todos, y valores más altos en los escenarios de desbalance acentuado. Tampoco aparece un punto de quiebre, puesto que el recorrido asciende hasta 1:50 y desciende en 1:100 sin describir una tendencia monótona. En consecuencia desaparece tanto el supuesto pico de inestabilidad en el escenario 1:50 como cualquier paradoja atribuida al escenario nativo, que se sitúa entre los valores altos y no se comporta de forma anómala. Este resultado es coherente con el hallazgo, desarrollado en el eje sintético, de que el desbalance es un factor secundario frente al explicador y la arquitectura.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{chapter_4/images_ch4/estabilidad_escenario.png}
    \caption{Estabilidad de las explicaciones por escenario de desbalance sobre el subgrafo receptivo, promediada sobre las tres semillas de modelo. La variación entre escenarios carece de dirección y no alcanza significancia estadística, con un valor p de 0,178 en la prueba de Kruskal-Wallis, lo que invalida la interpretación previa de un deterioro localizado.}
    \label{fig:escenario}
\end{figure}

\input{tables/elliptic_stab_scenario.tex}

La replicación con tres semillas se extendió también a GNNShap y a PGExplainer en dos de las cuatro arquitecturas, GCN y GraphSAGE. GNNShap reproduce su saturación en las tres semillas, con una correlación de Spearman de \textit{features} que promedia 0,950 en GraphSAGE y 0,971 en GCN, tal como ilustran la Figura \ref{fig:robustez} y la Tabla \ref{tab:elliptic-robustez-3s}. El soporte de ambas cifras es desigual y conviene declararlo. La de GraphSAGE se apoya en treinta y tres configuraciones, mientras que la de GCN descansa en solo cuatro, porque, como en el resto del estudio, los explicadores se aplican únicamente a las configuraciones que superan el filtro de calidad y muy pocas de GCN lo hacen. Ello ratifica que su altísima estabilidad convive con la incapacidad de ordenar arquitecturas, puesto que apenas las distingue. GAT y TAGCN quedaron fuera de esta extensión por una razón operativa y no metodológica. Sus modelos de las semillas 43 y 44, que forman parte de los ciento ochenta que sostienen la replicación de GNNExplainer, no estaban disponibles en el equipo en que se ejecutó la extensión, y el intento de reentrenarlos allí agotó la memoria de la tarjeta de ocho gigabytes en la primera configuración de GAT, lo que detuvo esa fase antes de llegar a TAGCN. Ambas arquitecturas se reportan, por tanto, sobre la semilla de referencia, en la que exhiben la misma saturación, con valores de 0,968 y 0,967 respectivamente. La observación resulta así transversal a las cuatro arquitecturas, aunque la replicación formal cubra solo dos. PGExplainer mantiene su comportamiento degenerado también entre semillas, con un índice de Jaccard de aristas próximo a la unidad en las cuatro arquitecturas, un artefacto de la extrema dispersión del grafo, en el que las escasas aristas del subgrafo receptivo se repiten casi idénticas entre réplicas y no porque el explicador capture una estructura estable.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{chapter_4/images_ch4/robustez_explicadores.png}
    \caption{Estabilidad de las explicaciones por arquitectura, medida como correlación de Spearman sobre el ranking de \textit{features}. GNNExplainer, en azul, distingue entre arquitecturas y reproduce la partición en dos grupos, con GAT y GCN por encima de GraphSAGE y TAGCN. GNNShap, en naranja, se satura en valores próximos a la unidad para las cuatro y no las separa. Las barras con trama y asterisco corresponden a GNNShap sobre GAT y TAGCN, reportadas sobre la semilla de referencia por la razón expuesta en el texto. La altura de cada barra de GNNExplainer es la media de todas las configuraciones agregadas, por lo que TAGCN aparece redondeado a 0,68, mientras que la Tabla \ref{tab:ic} informa 0,672 al promediar primero dentro de cada semilla. Ambas cifras proceden de los mismos datos y difieren solo en el orden de agregación.}
    \label{fig:robustez}
\end{figure}
