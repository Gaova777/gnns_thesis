...
La replicación con tres semillas se extendió también a GNNShap y a PGExplainer, para las dos arquitecturas cuyo reentrenamiento cabía en la GPU de ocho gigabytes disponible, GCN y GraphSAGE. GNNShap reproduce su saturación en las tres semillas, con una correlación de Spearman de \textit{features} que promedia 0,950 en GraphSAGE y 0,971 en GCN, con intervalos de confianza muy estrechos, tal como ilustran la Figura \ref{fig:robustez} y la Tabla \ref{tab:elliptic-robustez-3s}. Ello ratifica que su altísima estabilidad convive con la incapacidad de ordenar arquitecturas, puesto que apenas las distingue. GAT y TAGCN, cuyos pesos, con ocho cabezas de atención sobre el grafo completo, no cabían para reentrenarse en esa tarjeta, se reportan sobre la semilla de referencia, en la que exhiben la misma saturación, con valores de 0,968 y 0,967 respectivamente, de modo que la observación es transversal a las cuatro arquitecturas aunque la replicación formal cubra dos. PGExplainer mantiene su comportamiento degenerado también entre semillas, con un índice de Jaccard de aristas próximo a la unidad en las cuatro arquitecturas, un artefacto de la extrema dispersión del grafo, en el que las escasas aristas del subgrafo receptivo se repiten casi idénticas entre réplicas y no porque el explicador capture una estructura estable.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{chapter_4/images_ch4/robustez_explicadores.png}
    \caption{Estabilidad de las explicaciones por arquitectura, medida como correlación de Spearman sobre el ranking de \textit{features}. GNNExplainer, en azul, distingue entre arquitecturas y reproduce la partición en dos grupos, con GAT y GCN por encima de GraphSAGE y TAGCN. GNNShap, en naranja, se satura en valores próximos a la unidad para las cuatro y no las separa. Las barras con trama y asterisco corresponden a GNNShap sobre GAT y TAGCN, reportadas sobre la semilla de referencia por la limitación de memoria ya descrita.}
    \label{fig:robustez}
\end{figure}
