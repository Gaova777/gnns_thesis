...
Los modelos entrenados alcanzan un rendimiento razonable sobre la partición de validación, con un área bajo la curva de precisión y exhaustividad cercana a 0,339 en la mejor configuración hallada por la búsqueda de hiperparámetros. Sin embargo, ese desempeño no se traslada a la partición de test, donde el área bajo la misma curva cae a un valor medio cercano a 0,013 y el F1 de la clase ilícita se sitúa alrededor de 0,012. La magnitud de esta caída es tan pronunciada que ninguna arquitectura escapa a ella, como ilustran los valores de F1 en test por arquitectura, de 0,0245 para GAT, 0,0133 para GCN, 0,0077 para GraphSAGE y 0,0015 para TAGCN, que la Tabla \ref{tab:elliptic-perf} recoge junto con el coeficiente de correlación de Matthews y el área bajo la curva de precisión y exhaustividad. Este fenómeno, que puede denominarse colapso de validación a test, constituye uno de los rasgos más importantes del dominio y condiciona la interpretación de todo el estudio.
...
\section{Del Grafo Completo al Subgrafo Receptivo: una Corrección Metodológica}
...
Este comportamiento generaba un artefacto grave que conviene exponer con detalle. El ranking de estabilidad de la versión anterior situaba a GAT como la arquitectura más estable, con un valor cercano a 0,534, por encima de GraphSAGE, que aparecía como la peor con 0,345, pero esa comparación no era legítima porque GAT se medía sobre un subconjunto favorable de sus configuraciones mientras GraphSAGE se medía sobre todas. La corrección consiste en calcular cada explicación sobre el subgrafo receptivo del nodo de interés, es decir el conjunto de nodos que realmente influyen en su predicción según el número de capas de la arquitectura, en lugar de sobre el grafo completo. Para TAGCN, cuyo campo receptivo se amplía por el orden del filtro polinomial, se emplea un radio de dos saltos. Esta restricción reduce el consumo de memoria de 7,5 GiB a 0,02 GiB y elimina por completo los fallos, de manera que las cuatro arquitecturas se miden ahora sobre el mismo soporte y en condiciones equitativas.

Al recalcular la estabilidad sobre el subgrafo receptivo, el ranking se invierte respecto de la versión anterior. Sobre el conjunto completo de configuraciones, GraphSAGE alcanza la mayor estabilidad con 0,630, seguido de GAT con 0,486 y GCN con 0,468, muy próximos entre sí, mientras que TAGCN queda en último lugar con 0,270, como muestra la Figura \ref{fig:ranking}. Sobre el subconjunto de veintitrés configuraciones que superan el filtro de calidad, el mismo que utiliza la tesis para sus conclusiones, GraphSAGE conserva el primer lugar con 0,640 sobre once configuraciones, seguido de GCN con 0,639 sobre una sola, GAT con 0,535 sobre siete y TAGCN con 0,363 sobre cuatro, tal como recoge la Tabla \ref{tab:ranking}. La prueba de que el ranking anterior era un artefacto resulta directa, ya que sobre este mismo subconjunto GAT apenas se movió, de 0,534 a 0,535, mientras que GraphSAGE saltó de 0,345 a 0,640 al medirse por fin sobre todas sus configuraciones.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{chapter_4/images_ch4/ranking_khop.png}
    \caption{Estabilidad de las explicaciones por arquitectura sobre el subgrafo receptivo, en la corrida completa y en la corrida con filtro de calidad. GraphSAGE encabeza ambas corridas, mientras que GAT apenas varía entre subconjuntos, lo que evidencia que el ranking previo favorable a GAT era un artefacto del cálculo sobre el grafo completo.}
    \label{fig:ranking}
\end{figure}

La lectura que esta tesis afirma como robusta es deliberadamente prudente. La única afirmación que se sostiene en ambas corridas es que GraphSAGE es la arquitectura más estable, y la comparación firme es que GraphSAGE supera a GAT. GAT y GCN no son ordenables entre sí, dado que en la corrida completa sus valores de 0,486 y 0,468 son prácticamente indistinguibles. En la corrida con filtro, además, GCN se apoya en una única configuración y TAGCN en cuatro, un soporte insuficiente para ordenarlos con confianza, de modo que sus valores se reportan pero no se utilizan para establecer un orden. Esta cautela es coherente con el número reducido de configuraciones que superan el filtro y evita sobreinterpretar diferencias que carecen de respaldo estadístico.

El detalle por configuración, que el anexo recoge en su totalidad, permite apreciar el origen de esta cautela. La estabilidad varía de forma apreciable entre configuraciones de una misma arquitectura según el escenario y el balanceo, y el número de verdaderos positivos disponibles para medir la estabilidad oscila también de forma notable, lo que hace que algunas medias por arquitectura descansen sobre un soporte más sólido que otras. GraphSAGE se beneficia de disponer de un número mayor de configuraciones con calidad garantizada, lo que refuerza la confianza en su primer lugar, mientras que GCN y TAGCN, con muy pocas configuraciones en la corrida con filtro, no ofrecen un soporte suficiente para afirmar su posición relativa. Esta lectura, apoyada en la tabla completa, es la que justifica limitar la afirmación robusta a la superioridad de GraphSAGE sobre GAT y a la constatación de que TAGCN ocupa la posición más baja en la corrida completa.
...
\begin{table}[htbp]
    ...
    \midrule
    GraphSAGE & 0,630 (15) & 0,640 (11) \\
    GAT       & 0,486 (15) & 0,535 (7) \\
    GCN       & 0,468 (15) & 0,639 (1) \\
    TAGCN     & 0,270 (12) & 0,363 (4) \\
    \bottomrule
    \end{tabular}
    \caption{Estabilidad de las explicaciones (correlación de Spearman) por arquitectura sobre el subgrafo receptivo, con el número de configuraciones de soporte entre paréntesis. La lectura robusta es que GraphSAGE supera a GAT; GCN con una configuración y TAGCN con cuatro carecen de soporte para ordenarse en la corrida con filtro.}
    \label{tab:ranking}
\end{table}

El comportamiento de la estabilidad a lo largo de los escenarios de desbalance también difiere de lo que la versión anterior sugería. En lugar de un deterioro pronunciado en algún punto intermedio, el perfil es esencialmente plano, con valores que van de 0,525 en el escenario 1:1 a 0,433 en el escenario 1:10, pasando por 0,501 en 1:100, 0,470 en 1:50 y 0,439 en el escenario nativo, como ilustra la Figura \ref{fig:escenario} y detalla por arquitectura la Tabla \ref{tab:elliptic-stab-scen}. Este perfil plano tiene consecuencias que obligan a retractar afirmaciones de la versión anterior del estudio. En primer lugar, desaparece el supuesto pico de inestabilidad en el escenario 1:50, ya que ese punto no destaca sobre los demás. En segundo lugar, no existe la paradoja atribuida al escenario nativo, puesto que este queda a mitad de tabla y el valor más bajo corresponde al escenario 1:10.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{chapter_4/images_ch4/estabilidad_escenario.png}
    \caption{Estabilidad de las explicaciones por escenario de desbalance sobre el subgrafo receptivo. El perfil es esencialmente plano, sin un pico de inestabilidad en el escenario 1:50 y con el valor más bajo en el escenario 1:10, lo que invalida la interpretación previa de un deterioro localizado.}
    \label{fig:escenario}
\end{figure}

\input{tables/elliptic_stab_scenario.tex}

La corrección obliga además a retractar el supuesto compromiso entre exactitud y estabilidad que la versión anterior reportaba con una correlación cercana a -0,20. Ese compromiso presuponía que la arquitectura más precisa sacrificaba estabilidad, pero una vez corregido el artefacto de memoria se observa que GraphSAGE encabeza tanto la estabilidad como el rendimiento relativo, de modo que no existe tal disyuntiva. Retractar estos hallazgos no debilita la tesis, sino que la fortalece, ya que sustituye conclusiones apoyadas en un artefacto por conclusiones apoyadas en una medición homogénea. Este episodio ilustra, además, un riesgo metodológico general en la evaluación de explicadores, el de que fallos silenciosos de cómputo se confundan con propiedades genuinas de las arquitecturas.
