...
En el contexto de detección AML, el DL ofrece dos ventajas fundamentales. La primera es la capacidad de aprender automáticamente características discriminativas desde datos transaccionales sin requerir conocimiento experto previo. La segunda es la flexibilidad arquitectónica para incorporar diferentes tipos de datos (atributos numéricos, secuencias temporales, estructuras de grafos) dentro de un marco unificado \cite{He2026AnDetection}.
...
Diversos estudios recientes reportan la ventaja de las GNNs en detección AML de forma verificada mediante experimentos controlados. Lawal et al. mostraron que los modelos basados en grafos superan de manera consistente a los enfoques tabulares en el \textit{Elliptic Dataset} \cite{Lawal2025AnDataset}. He et al. describieron un marco explicable en el que las GNNs capturan patrones estructurales que los métodos tabulares no logran recuperar \cite{He2026AnDetection}. Esta ventaja, sin embargo, no es incondicional, ya que su magnitud depende de la densidad del grafo, de la calidad de las conexiones observadas y de la arquitectura concreta empleada, un punto que los resultados de esta tesis abordan de manera directa.
...
Las \textit{Graph Attention Networks}, introducidas por Velickovic y colaboradores, incorporan mecanismos de atención que permiten asignar pesos aprendibles a diferentes vecinos, superando la limitación isotrópica de GCN \cite{Velickovic2018GraphNetworks}.
...
\input{chapter_3/propiedades_gnn.tex}
...
\begin{itemize}
    \item \textbf{GNNExplainer}: Propuesto por Ying y colaboradores, formula la explicación como un problema de optimización que identifica el subgrafo más compacto que maximiza la predicción del modelo para la clase objetivo \cite{Ying2019Gnnexplainer:Networks}. Matemáticamente:
...
Aunque estas métricas proporcionan información valiosa sobre el comportamiento del modelo, evaluarlas de manera aislada o depender de la exactitud global (\textit{accuracy}) resulta altamente engañoso en contextos de desbalance extremo (por ejemplo, 1:1000). En estos escenarios, un clasificador trivial que prediga siempre la clase mayoritaria obtendría una exactitud casi perfecta y una alta especificidad, pero su utilidad operativa para detectar el lavado de dinero sería nula. De igual forma, centrarse únicamente en maximizar la precisión suele reducir drásticamente la exhaustividad, dejando pasar operaciones ilícitas críticas \cite{Rai2025EvaluatingData}.
...
Las tres semillas de modelo empleadas en esta tesis son los enteros consecutivos 42, 43 y 44, y la forma de elegirlas importa tanto como su número. El valor 42 se fija como semilla de referencia por ser el adoptado de manera convencional en la práctica del aprendizaje automático, donde funciona como opción por defecto habitual en tutoriales, bibliotecas y publicaciones. Se adopta aquí por reproducibilidad y no por su efecto sobre los resultados, mientras que 43 y 44 son sus sucesores inmediatos. La decisión es, por tanto, íntegramente anterior a la ejecución y puramente mecánica, ya que no se ensayaron varias semillas para conservar las más favorables ni se aplicó entre ellas criterio de selección alguno. Esta regla determinista, anunciada de antemano, elimina el grado de libertad que abriría la puerta a un sesgo de selección, de modo que ninguna conclusión de este trabajo puede atribuirse a haber hallado una inicialización afortunada. En coherencia con este principio, las conclusiones sustantivas se sostienen sobre el agregado de las tres semillas, y cuando una cifra se reporta sobre una sola, se trata siempre de la semilla 42 en su papel de referencia. Ese valor no se da por representativo de antemano, sino que se contrasta con las otras dos semillas allí donde existen, y el contraste no siempre lo favorece, como muestra el caso de TAGCN, cuya estabilidad en la semilla 42 cae en el extremo bajo de su rango, según documenta el Capítulo 4. Precisamente porque la semilla de referencia puede quedar en un extremo, ninguna conclusión sobre el orden entre arquitecturas descansa en ella sola.
