...
\input{tables/elliptic_full.tex}
...
La disponibilidad de los datos y del código sigue este mismo principio. El \textit{Elliptic Dataset} es de acceso público y la implementación lo descarga de forma automática en la primera ejecución. El grafo sintético no se distribuye como un archivo cerrado, sino que se regenera de forma determinista a partir del generador incluido y de la semilla del generador, y los resultados agregados por configuración que sustentan las tablas y figuras del Capítulo 5 se conservan versionados junto al código. El código de entrenamiento, de los tres explicadores, del cálculo de estabilidad y del análisis estadístico está disponible en el repositorio del proyecto, mientras que los pesos entrenados no se versionan por su tamaño, dado que son regenerables a partir del \textit{pipeline} y de las semillas declaradas. De este modo, un tercero puede reproducir el eje sintético en su totalidad y reejecutar el eje Elliptic desde el conjunto de datos público, con la única salvedad de la no reproducibilidad exacta a nivel de pesos ya descrita.
