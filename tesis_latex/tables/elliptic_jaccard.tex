\begin{table}[htbp]
\centering
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{lcc}
\toprule
\textbf{Arquitectura} & \textbf{Jaccard medio} & \textbf{Spearman medio} \\
\midrule
GraphSAGE & 0,911 & 0,735 \\
GAT & 0,791 & 0,780 \\
GCN & 0,613 & 0,778 \\
TAGCN & 0,805 & 0,580 \\
\bottomrule
\end{tabular}
\caption{Comparación entre el índice de Jaccard de aristas y la correlación de Spearman de features (GNNExplainer) como metricas de estabilidad sobre el \textit{Elliptic Dataset}, con la métrica corregida. El Jaccard se satura hacia valores altos por la dispersion del grafo, mientras que la correlación de Spearman conserva capacidad de discriminacion entre arquitecturas.}
\label{tab:jaccard}
\end{table}
